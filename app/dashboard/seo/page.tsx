        ...
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 8, padding: 14 }}>
          {node.position ? (
            <div style={{ fontSize: 13, color: 'var(--text)', lineHeight: 1.9 }}>
              <span style={{ color: '#ff8c42', fontWeight: 700 }}>{Math.max(1, 18 - (node.position || 18))} backlinks</span> from DA 30+ sites<br/>
              <span style={{ color: '#ffe600', fontWeight: 700 }}>~{Math.ceil((node.position || 10) * 1.4)} months</span> at current pace<br/>
              <span style={{ color: '#c084fc', fontWeight: 700 }}>+{Math.ceil((node.position || 10) * 120)} words</span> supporting content needed
            </div>
  ...
  function renderNode(node: any, depth: number = 0): any {
    ...
    return (
      <div key={node.id || node.nodeId} style={{ marginLeft: depth * 20, position: 'relative' }}>
        {depth > 0 && (
          <div style={{ position: 'absolute', left: -16, top: 18, width: 12, height: 1,
            background: 'rgba(255,255,255,0.1)' }} />
        )}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          <div onClick={() => setSelectedNode(node)}
            style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, padding: '7px 12px',
              ...
              opacity: node.status === 'planned' ? 0.5 : 1,
          ...
          {children.length > 0 && (
            <button onClick={() => setExpanded(s => { const n = new Set(s); const id = node.id || node.nodeId; n.has(id) ? n.delete(id) : n.add(id); return n; })}
...
export default function SeoPage() {
  ...
  useEffect(() => { loadKeywords(); loadStructure(); }, []);

  async function loadKeywords() {
    try { const r = await fetch('/api/seo/keywords'); if (r.ok) { const d = await r.json(); setKeywords(d.keywords || []); } } catch {}
  }
  async function loadStructure() {
    try { const r = await fetch('/api/seo/structure'); if (r.ok) { const d = await r.json(); setNodes(d.nodes || []); setLinks(d.links || []); } } catch {}
  }
  async function addKeyword() {
    setSaving(true);
    try {
      await fetch('/api/seo/keywords', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      ...
      setShowAddKw(false); loadKeywords();
  ...
  return (
    <div>
      ...
      <div className="stats-grid" style={{ marginBottom: 24 }}>
        <KpiCard label="Avg Position" value={current.avgPos} sub="vs prev period" color="var(--green)" prev={previous.avgPos} tooltip="Average ranking across all tracked keywords. Lower = better. Green arrow means you improved." />
        ...
        <KpiCard label="Impressions" value={(current.impressions/1000).toFixed(1)+'K'} sub="Search impressions" color="var(--yellow)" prev={previous.impressions} tooltip="How many times NSS appeared in Google results. Even without clicks, impressions build brand awareness." />
        <KpiCard label="Avg CTR" value={current.ctr+'%'} sub="Click-through rate" color="var(--pink)" prev={previous.ctr} tooltip="% of people who clicked after seeing NSS in Google. Higher CTR means titles and descriptions are compelling." />
      </div>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ display: 'flex', gap: 4 }}>
              {PERIODS.map(p => (
                <button key={p.key} onClick={() => setPeriod(p.key)} className={period === p.key ? 'btn btn-pink' : 'btn btn-ghost'} style={{ fontSize: 11, padding: '5px 10px' }}>{p.label}</button>
              ))}
            </div>
            <button onClick={() => setComparePeriod(!comparePeriod)} className={comparePeriod ? 'btn btn-pink' : 'btn btn-ghost'} style={{ fontSize: 11, padding: '5px 12px', display: 'flex', alignItems: 'center', gap: 5 }}>
        ...
        <ResponsiveContainer width="100%" height={240}>
          <LineChart data={trendData}>
            ...
            <Line type="monotone" dataKey="current" stroke="#a8cf45" strokeWidth={2.5} dot={{ fill: '#a8cf45', strokeWidth: 0, r: 3 }} activeDot={{ r: 6, filter: 'url(#glow)' }} animationDuration={800} filter="url(#glow)" />
            {comparePeriod && <Line type="monotone" dataKey="previous" stroke="#04aae8" strokeWidth={1.5} strokeDasharray="5 3" dot={false} activeDot={{ r: 4 }} opacity={0.6} animationDuration={800} />}
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input className="form-input" placeholder="Search keywords…" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} style={{ width: 180, padding: '6px 12px', fontSize: 12 }} />
            <button onClick={() => setShowAddKw(!showAddKw)} className="btn btn-ghost" style={{ fontSize: 12, padding: '6px 12px' }}>+ Add Keyword</button>
          </div>
        </div>
        ...
        {showAddKw && (
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', borderRadius: 10, padding: 16, marginBottom: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 2fr 1fr 1fr 1fr', gap: 10, marginBottom: 12 }}>
              <div><label className="form-label">Keyword *</label><input className="form-input" placeholder="e.g. wax strips australia" value={addKwForm.keyword} onChange={e => setAddKwForm(f => ({ ...f, keyword: e.target.value }))} /></div>
              <div><label className="form-label">Page URL</label><input className="form-input" placeholder="/waxing/wax-strips/" value={addKwForm.url} onChange={e => setAddKwForm(f => ({ ...f, url: e.target.value }))} /></div>
              ...
              <div><label className="form-label">Volume/mo</label><input className="form-input" type="number" placeholder="1200" value={addKwForm.volume} onChange={e => setAddKwForm(f => ({ ...f, volume: e.target.value }))} /></div>
              <div><label className="form-label">Position</label><input className="form-input" type="number" placeholder="8" value={addKwForm.position} onChange={e => setAddKwForm(f => ({ ...f, position: e.target.value }))} /></div>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={addKeyword} className="btn btn-pink" style={{ fontSize: 12 }} disabled={saving}>{saving ? 'Saving…' : 'Add Keyword'}</button>
          ...
          <table className="data-table">
            ...
            <tbody>
              {displayed.map((r: any) => {
                ...
                const isDown = diff < 0;
                return (
                  <tr key={r.id || r.keyword} style={{ background: isTop ? 'rgba(168,207,69,0.04)' : isDown ? 'rgba(255,80,80,0.03)' : 'transparent' }}>
                    <td style={{ fontWeight: 500 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                        {isTop && <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#a8cf45', boxShadow: '0 0 6px #a8cf45', flexShrink: 0 }} />}
                        {isDown && <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#ff5050', flexShrink: 0 }} />}
                        {r.keyword}
                      </div>
                    </td>
                    <td><PosBadge pos={r.position} /></td>
                    <td>{(() => { const d = (r.prev_position || r.prev || r.position) - r.position; if (d > 0) return <span style={{ color: 'var(--green)', fontWeight: 700 }}>↑{d}</span>; if (d < 0) return <span style={{ color: '#ff5050', fontWeight: 700 }}>↓{Math.abs(d)}</span>; return <span style={{ color: 'var(--muted)' }}>—</span>; })()}</td>
                    <td style={{ color: 'var(--muted)' }}>{(r.volume || 0).toLocaleString()}/mo</td>
                    <td>{r.url ? <span style={{ fontSize: 11, color: 'var(--blue)', background: 'rgba(4,170,232,0.08)', padding: '2px 6px', borderRadius: 4, fontFamily: 'monospace' }}>{r.url}</span> : <span style={{ color: 'var(--muted)', fontSize: 11 }}>—</span>}</td>
                    <td><span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 12, background: 'rgba(255,255,255,0.05)', color: 'var(--muted)', border: '1px solid var(--border)' }}>{r.category || 'Uncategorized'}</span></td>
                  </tr>
                );
              })}
            </tbody>
